...
testdb <- NULL
my_directory <- system.file(package = 'sensitivityselectioncriteria')
my_file_folder <- system.file('extdata', package = 'sensitivityselectioncriteria')
...
mk_testdb_omop <- function(){
  ...
  for (file_name in list.files(path=system.file('extdata',
    ...
    tbl <- readr::read_csv(
      file = file.path(system.file('extdata', package = 'sensitivityselectioncriteria'),
                       file_name),
